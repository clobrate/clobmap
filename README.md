...
> **Status:** Phase 11 / 15 — release pipeline automated. Tag `v0.x.y` → GitHub Actions matrix-builds macOS arm + intel + Windows + Linux, signs and notarizes the macOS bundles, signs every platform's updater artifact, assembles `latest.json`, and promotes the draft release once every job succeeds. Release runbook at [`RELEASING.md`](./RELEASING.md). Live at https://clobmap.com.
